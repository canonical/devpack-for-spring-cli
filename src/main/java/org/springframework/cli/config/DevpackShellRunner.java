...
public class DevpackShellRunner implements ShellRunner {
	...
	public void run(String[] args) throws Exception {
		try {
			if (ObjectUtils.isEmpty(args)) {
				throw new ShellExitException(new ExitStatus(1,
	...
	private void executeScript(File script) {
		try (FileInputProvider inputProvider = new FileInputProvider(script)) {
			...
			throw new ShellExitException(new ExitStatus(1, "Unable to locate script file: " + e.getMessage()));
		}
	}
	...
	private void executeScript(InputProvider inputProvider) {
		while (true) {
			...
			try {
				...
				throw new ShellExitException(new ExitStatus(1, "Unable to read command: " + e.getMessage()));
			}
			if (input == null) {
